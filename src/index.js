...
window.Webflow.push(() => {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const billedWeightUSPS = calculateUSPSFirstmileBilledWeight(actualWeightInput, dimWeightUSPS, cubicSize, isPounds);
        const billedWeightFirstmile = calculateUSPSFirstmileBilledWeight(actualWeightInput, dimWeightFirstmile, cubicSize, isPounds);
        const billedWeightUPS = calculateUPSFedExBilledWeight(actualWeightInput, dimWeightUPS, isOunces);
        const billedWeightFedEx = calculateUPSFedExBilledWeight(actualWeightInput, dimWeightFedEx, isOunces);
        ...
        if (actualWeight !== null) {
            ...
            if (isPounds) {
                ...
                if (length !== null && width !== null && height !== null) {
                    const cubicSize = length * width * height;
                    const dimWeightUSPS = calculateDimensionalWeight(cubicSize, 166);
                    const billedWeightUSPS = calculateUSPSFirstmileBilledWeight(actualWeight, dimWeightUSPS, cubicSize, isPounds);
                    const billedWeightFirstmile = calculateUSPSFirstmileBilledWeight(actualWeight, dimWeightUSPS, cubicSize, isPounds);
                    document.querySelector('#dim-weight-usps').textContent = `${dimWeightUSPS} lbs`;
